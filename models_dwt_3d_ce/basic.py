...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, in_channel=1, zero_init_residual=False):
        ...
        self.conv1 = nn.Conv3d(in_channel, 64, kernel_size=3, stride=1, padding=1,
                               bias=False)
...
class LinearBatchNorm(nn.Module):
    """Implements BatchNorm1d by BatchNorm3d, for SyncBN purpose"""
    def __init__(self, dim, affine=True):
        super(LinearBatchNorm, self).__init__()
        self.dim = dim
        self.bn = nn.BatchNorm3d(dim, affine=affine)
...
class SupCEResNet(nn.Module):
    """encoder + classifier"""
    def __init__(self, name='resnet18', normalize = False,  num_classes=12):
...
class SupCEHeadResNet(nn.Module):
    ...
    def __init__(self, args, multiplier = 1):
        ...
        model_fun, dim_in = model_dict[args.arch]
